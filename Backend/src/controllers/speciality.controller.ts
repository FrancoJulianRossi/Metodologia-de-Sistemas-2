...
class speacialityController {
    async getAllSpecialities(req: any, res: any) {
        try {
            ...
            res.status(500).json({
                error: error instanceof Error ? error.message : "Error retrieving specialities",
            });
        }
    }
...
async getSpecialityById(req: any, res: any) {
    try {
        ...
        res.status(404).json({
            error: error instanceof Error ? error.message : "Error retrieving speciality",
        });
    }
}
...
async createSpeciality(req: any, res: any) {
    try {
        ...
        res.status(500).json({
            error: error instanceof Error ? error.message : "Error creating speciality",
        });
    }
}
...
async updateSpeciality(req: any, res: any) {
    try {
        const id = parseInt(req.params.id);
        const specialityData = req.body;
        const updatedSpeciality = await specialityModel.update(id, specialityData);
        res.status(200).json(updatedSpeciality);
    } catch (error: any) {
        console.error("Error updating speciality:", error);
        res.status(404).json({
            error: error instanceof Error ? error.message : "Error updating speciality",
        });
    }
}
...
async deleteSpeciality(req: any, res: any) {
    try {
        ...
        res.status(404).json({
            error: error instanceof Error ? error.message : "Error deleting speciality",
        });
    }
}
}
